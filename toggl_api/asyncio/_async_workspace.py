...
class AsyncWorkspaceEndpoint(TogglAsyncCachedEndpoint[TogglWorkspace]):
    ...
    async def get(
        ...
        *,
        ...
        try:
            response = await self.request(f"workspaces/{workspace}", refresh=refresh)
    ...
    async def _collect_cache(self, since: int | None) -> ScalarResult[TogglWorkspace]:
        statement = select(TogglWorkspace)
        if isinstance(since, int):
            ts = datetime.fromtimestamp(since, timezone.utc)
            statement = statement.filter(cast(ColumnElement[bool], TogglWorkspace.timestamp > ts))
    ...
    async def collect(
        ...
        *,
        ...
        response = await self.request("me/workspaces", body=body, refresh=refresh)
    ...
    async def statistics(self, workspace_id: TogglWorkspace | int) -> WorkspaceStatistics:
        ...
        response = await self.request(f"workspaces/{workspace_id}/statistics", refresh=True, raw=True)
